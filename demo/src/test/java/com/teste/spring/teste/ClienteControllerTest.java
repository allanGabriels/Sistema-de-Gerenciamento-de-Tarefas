...
public class ClienteControllerTest {
    ...
    void deveCriarCliente() throws Exception {
        ...
        String email = "maria.silva@teste.com";
        String telefone = "11998877665";
        Long idSalvo = 20L;

        // 1. Simular o Cliente que o Service deve retornar (com ID gerado)
        ...
        // 2. Definir o comportamento do mock service
        // Quando service.criar for chamado com qualquer Cliente, retorne o Cliente salvo
        when(service.criar(any(Cliente.class))).thenReturn(salvo);

        // 3. Payload JSON de entrada (DTO)
        // O DTO é passado como JSON no corpo da requisição
        String jsonRequest = "{\"nome\":\"" + nome + "\", \"email\":\"" + email + "\", \"telefone\":\"" + telefone + "\"}";

        // 4. Executar a requisição POST simulada
        mvc.perform(post("/api/clientes")
                        .contentType(MediaType.APPLICATION_JSON) // Tipo de conteúdo da requisição
                        .content(jsonRequest) // Corpo da requisição
                        .accept(MediaType.APPLICATION_JSON))
                // 5. Verificar status HTTP e conteúdo da resposta
                .andExpect(status().isCreated()) // Espera status 201 Created
                .andExpect(jsonPath("$.id").value(idSalvo)) // Verifica o ID retornado
                .andExpect(jsonPath("$.nome").value(nome))
                .andExpect(jsonPath("$.email").value(email));

        // 6. Verificar se o método criar do service foi chamado exatamente uma vez
